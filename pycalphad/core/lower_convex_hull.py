        ...
        for coord_idx, str_cond_key in enumerate(sorted(result_array.coords.keys())):
            ...
            if isinstance(cond_key, SiteFraction):
                ...
                if basis_is_trivial:
                    ...
                    coef_vector = np.multiply(coef_vector, molar_masses)
                else:
                ...
                if basis_is_trivial:
                    ...
                    # X(c) = k <=> ((S^T)^-1[c,:] - k*colsum) . x_elem = 0  (homogeneous)
                    row = basis_row(cond_key.species)
                    idx_fixed_lincomb_molefrac_coefs.append(component_basis_inv_T[row] - rhs * inv_T_colsum)
                ...
                # components; component N(i) selects one. Collect the selector vector + value;
                # they become scale-invariant ratio rows after the loop (see below). Total moles
                # exposes no `species` attribute, so probe defensively with getattr.
                ...
                if basis_is_trivial:
                    ...
                    # (see solver.get_system_spec for the derivation).
                    constant = 0.0
                    for symbol_idx, symbol in enumerate(cond_key.symbols):
                        if symbol == 1:
